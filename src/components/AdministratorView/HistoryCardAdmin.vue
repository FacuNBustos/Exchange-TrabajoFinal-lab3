...
<template>
    ...
    <div class="flex flex-row items-center w-full h-12 bg-blue-200 mt-1 p-2 border shadow-sm shadow-gray-300 rounded">
        ...
        <div class="flex flex-row items-center w-[70%]">
            <h2 class="text-roboto text-sm text-gray-700 mr-3">ID: <span class="text-black">{{history._id}}</span></h2>
            <h3 class="text-roboto text-sm text-gray-700 mr-2">Fecha: <span class="text-black">{{new Date(history.datetime).toLocaleDateString()}}</span></h3>
            <h4 class="text-roboto text-sm text-gray-700 mr-2">Moneda: <span class="text-black">{{history.crypto_code.toUpperCase()}}</span></h4>
            <h4 class="text-roboto text-sm text-gray-700 mr-4">Cantidad: <span class="text-black">{{history.crypto_amount}}</span></h4>
            <h4 :class="(history.action == 'purchase')? 'text-roboto text-lg mr-2 text-green-700'
            : 'text-roboto text-lg mr-2 text-red-700'">
            {{(history.action == 'purchase')? 'Compra' : 'Venta'}}
            </h4>
        </div>
