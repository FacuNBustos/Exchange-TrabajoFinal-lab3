...
<template>
    ...
    <div class="flex flex-col items-center w-[1000px] h-[650px] rounded shadow-sm shadow-black bg-white p-2">
        ...
        <div class="flex flex-col w-full h-[580px] mt-4 border border-gray-300">
            <div class="flex-grow overflow-auto pl-6 pr-6 text-center">
                ...
                <h3 v-if="!$store.state.admin.search" class="text-roboto text-lg text-gray-600 mt-3">
                No hay registros aparentes
                </h3>
                
                <HistoryCardAdmin v-if="$store.state.admin.search" v-for="elem in $store.state.admin.search" :history='elem'/>
    ...
    <div v-if="$store.state.admin.editCheck" 
    class="flex flex-col justify-center items-center absolute w-[1200px] h-[700px] bg-gray-0 rounded">
        <div class="flex flex-col justify-center items-center w-[600px] h-[300px] p-2 bg-white gap-y-2 rounded-t">
            ...
            <h2 class="text-roboto "><span class="underline">ID transaccion:</span> {{$store.state.admin.editableInfo._id.toUpperCase()}}</h2>
            <h3 class="text-roboto"><span class="underline">Fecha y Hora:</span> {{new Date($store.state.admin.editableInfo.datetime).toLocaleString()}}</h3>
            <h3 class="text-roboto"><span class="underline">Moneda:</span> {{$store.state.admin.editableInfo.crypto_code.toUpperCase()}}</h3>
            <h3 :class="($store.state.admin.editableInfo.action == 'purchase')? 'text-roboto text-lg mr-2'
            : 'text-roboto text-lg mr-2'">
            <span class="underline">Tipo:</span> {{($store.state.admin.editableInfo.action == 'purchase')? 'Compra' : 'Venta'}}
            </h3>
            <label for="" class="text-roboto">
                <span class="underline mr-2">Cantidad:</span>
                <input type="text" id="money" :value="$store.state.admin.editableInfo.crypto_amount" class="text-center border border-gray-200">
            </label>
            <label for="" class="text-roboto">
                <span class="underline mr-1">Monto de transferencia:</span> $
                <input type="text" id="amount" :value="$store.state.admin.editableInfo.money" class="text-center border border-gray-200">
            </label>
